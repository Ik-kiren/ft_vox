...
void Sun::UpdatePosition() {
    float revolutionTimer = std::fmod(glfwGetTime() / ROTATIONSPEED, 2 * std::numbers::pi);
    ...
    model = Rotate(model, ((2* std::numbers::pi * (60 /((2* std::numbers::pi) * ROTATIONSPEED))) / 60) / (1/timer), Vector3(0, 0, 1));
