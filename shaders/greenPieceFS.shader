...
void main()
{
	...
	vec3 result = (ambient + diffuse + specular) * newColor.xyz;
	...
	if (activeTexture)
		...
		fragText = vec4(mix(tmpTexture.x, result.x, timerTextureTransition), mix(tmpTexture.y, result.y, timerTextureTransition), mix(tmpTexture.z, result.z, timerTextureTransition), newColor.w);
}